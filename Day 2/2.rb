...
@facts_dict = {2 => [1], 3 => [1], 4 => [2], 5 => [5], 6 => [3,2], 7 => [1], 8 => [4], 9 => [3], 10 => [5,2]}

def main
  ranges_strings = AocUtils.read_strings('Day 2/testinput.txt').flatten
...
def split_range_into_ranges_with_same_digit_count(range)
  if range[0].to_s.length == range[1].to_s.length
    [range]
  end
